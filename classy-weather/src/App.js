...
class App extends React.Component {
  ...
  //useEffect, [location], but its not called on mount
  ...
  render() {
    return <div className='app'>
      ...
      {this.state.weather.weathercode && <Weather weather={this.state.weather} location={this.state.displayLocation}></Weather>}
    </div>
  }
}
...
class Weather extends React.Component {
  //Cleanup function but only works when component is destroyed
  ...
  render() {
    ...
    return <div>
      ...
      <ul className='weather'>
        {dates.map((date, i) => <Day date={date} max={max.at(i)} min={min.at(i)} code={codes.at(i)} isToday={i===0} key={date}/>)}
      </ul>
    </div>
  }
}
